...
void displayQRCode(String text) {
   // display.clearScreen();
    ...
    do {
        ...
        if (success) {
          qrcode.setScale(4);        
          int16_t sideLength = qrcode.getSideLength(); // Độ dài QR, dùng để căn chỉnh cái text bên phải kia cho đẹp
...
void loop() {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // Cấu hình đánh thức bằng GPIO 33 (mức thấp)
        esp_sleep_enable_ext0_wakeup(GPIO_NUM_33, 0);
        Serial.println("Đi ngủ zzz...");
        delay(1000);
